...
class LoginController extends PublicController {
	 ...
	 public function _initialize() {
        require_once './Public/plugins/alipay-sdk/AopSdk.php';
    }
	...
	//支付宝获取access_token
	public function a_getsessionkey(){
		$code = trim($_POST['code']);
		if (!$code) {
			echo json_encode(array('status'=>0,'err'=>'非法操作！'));
			exit();
		}
		// vendor('AlipaySDK.vendorFile');
		$appid = C('zhifubao.appid');
		$rsaPrivateKey = C('zhifubao.rsaPrivateKey');
		$alipayrsaPublicKey = C('zhifubao.alipayrsaPublicKey');
		$aop = new \AopClient();
		...
		$resultCode = $result->$responseNode->access_token;
		// dump($result->$responseNode);exit;
		if(!empty($resultCode)){
			echo json_encode(array('status'=>1,'info'=>$result->$responseNode));
			exit();
		}else {
	...
	//支付宝获取用户信息
	public function a_getUserMessage(){
