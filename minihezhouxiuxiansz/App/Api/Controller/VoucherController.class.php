...
class VoucherController extends PublicController {
    ...
    public function getQuan2(){
        ...
        foreach($cartId as $k => $v){
            $pid[] = M('shopping_char')->where('id="'.intval($v).'"')->getField('pro_id');
        ...
        foreach($quan as $k3 => $v3){
            $temp2 = M('voucher')->where('id="'.intval($v3['vid']).'" AND cate_id="'.'all'.'"  AND full_money<"'.$total.'"')->select();
            if($temp2){
                $quan2 = array_merge($quan2,$temp2);
            }
